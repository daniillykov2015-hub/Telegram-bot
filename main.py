...
# ================== TERMS (FULL TEXT FIXED) ==================
@router.callback_query(F.data == "terms")
async def terms(call: CallbackQuery):

    text1 = """Пользовательское соглашение
...
3.4. Все решения о применении материалов принимаются Пользователем самостоятельно и на его риск.



4. Законность использования
4.1. Сервис не предназначен для противоправной деятельности.

4.2. Пользователь обязуется использовать Сервис в рамках законодательства.

4.3. Ответственность полностью на Пользователе.
"""

    text3 = """
5. Интеллектуальная собственность
5.1. Все материалы защищены законом.

5.2. Запрещено копирование и распространение.

5.3. Нарушение ведёт к ограничению доступа.



6. Ограничение доступа
6.1. Администрация вправе ограничить доступ.

6.2. Обязательства остаются.

6.3. Возможен отказ в обслуживании.



7. Платежи и возвраты
7.1. Оплата до доступа.

7.2. Возврат не предусмотрен.

7.3. Исключения — если услуга не оказана.

7.4. Обращение — в течение 24 часов.

7.5. Решение индивидуально.

7.6. Запрещён chargeback без обращения.
"""

    text4 = """
8. Конфиденциальность
8.1. Сервис собирает минимальные данные.

8.2. Без полной гарантии безопасности.
...
9.2. Актуальная версия в сервисе.

9.3. Использование = согласие.



10. Контактная информация
10.1. Через поддержку в боте.



Используя Сервис, пользователь принимает условия полностью.
"""

    await call.message.answer(text1)
    await call.message.answer(text2)
    await call.message.answer(text3)
    await call.message.answer(
        text4,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅ Назад", callback_data="info")]
